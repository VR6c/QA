...
export default function ProfileModal() {
  ...
  useEffect(() => {
    ...
    // Reset password form fields whenever modal opens
    setCurrentPassword('');
    setNewPassword('');
    setConfirmPassword('');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl shadow-2xl border border-slate-100 w-full max-w-lg max-w-[calc(100vw-2rem)] overflow-hidden flex flex-col max-h-[90vh]">
          ...
          <div className="flex items-center gap-3 mb-4">
            <div className="p-2.5 bg-white/15 rounded-2xl backdrop-blur-md">
              {activeProfileTab === 'password' ? (
                <LuKey className="w-6 h-6 text-white" />
              ) : (
            ...
            <div>
              ...
              <p className="text-xs text-blue-100 font-medium">Manage your personal profile and account security</p>
            </div>
          </div>

          {/* Navigation Tabs (Profile vs Change Password) */}
          <div className="flex bg-black/20 p-1 rounded-xl gap-1 backdrop-blur-sm">
            <button
              type="button"
        ...
        {activeProfileTab === 'profile' ? (
          ...
          <form onSubmit={handleProfileSubmit} className="p-6 overflow-y-auto space-y-6 flex-1">
          ...
          <form onSubmit={handlePasswordSubmit} className="p-6 overflow-y-auto space-y-5 flex-1">

            {/* Security Callout */}
            <div className="flex items-start gap-3 p-3.5 rounded-2xl bg-blue-50/70 border border-blue-100 text-blue-900">
              <LuLock className="w-5 h-5 text-blue-600 shrink-0 mt-0.5" />
              <div className="text-xs leading-relaxed">
                <p className="font-bold text-blue-950">Security Recommendation</p>
                <p className="text-blue-700/90 mt-0.5">
                  Choose a unique password at least 6 characters long to keep your account protected.
                </p>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-10 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-900 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
                  required
                />
                <button
                  type="button"
              ...
              {newPassword && (
                <div className="mt-2 space-y-1">
                  <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden flex gap-1">
                    <div className={`h-full transition-all duration-300 ${passwordStrength.score >= 1 ? passwordStrength.color.split(' ')[0] : 'bg-slate-200'} flex-1 rounded-full`} />
                    <div className={`h-full transition-all duration-300 ${passwordStrength.score >= 2 ? passwordStrength.color.split(' ')[0] : 'bg-slate-200'} flex-1 rounded-full`} />
                    <div className={`h-full transition-all duration-300 ${passwordStrength.score >= 3 ? passwordStrength.color.split(' ')[0] : 'bg-slate-200'} flex-1 rounded-full`} />
                  </div>
                </div>
              )}
            </div>
            ...
            <div>
                  ...
                  <div className="flex items-center gap-1 text-[11px]">
                    {newPassword === confirmPassword ? (
                      <span className="text-emerald-600 font-semibold flex items-center gap-1">
                        <LuCircleCheck className="w-3.5 h-3.5" /> Passwords match
                      </span>
                    ) : (
                      <span className="text-rose-500 font-semibold flex items-center gap-1">
                        <LuCircleAlert className="w-3.5 h-3.5" /> Does not match
                      </span>
                    )}
                  </div>
                )}
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full pl-9 pr-10 py-2.5 bg-slate-50 border rounded-xl text-sm font-medium text-slate-900 focus:bg-white focus:outline-none focus:ring-2 transition-all ${
            ...
            <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-100">
              ...
              <button
                ...
                className="flex items-center gap-2 px-5 py-2 text-xs font-bold text-white bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 rounded-xl shadow-md hover:shadow-lg transition-all cursor-pointer disabled:opacity-50"
              >
                {(isSubmitting || isLoading) && <LuLoader className="w-4 h-4 animate-spin" />}
                Update Password
              </button>
